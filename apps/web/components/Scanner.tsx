    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
        ...
        {(loading || progressStats.percent > 0) && (
          <div className="px-8 pt-8 pb-4 bg-indigo-50/30 border-b border-indigo-100">
        ...
        <div className="p-8">
          {selectedFiles.length === 0 ? (
            <div
              onClick={() => fileInputRef.current?.click()}
              className="border-2 border-dashed border-indigo-200 rounded-3xl p-16 flex flex-col items-center gap-6 cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/50 transition-all group"
            >
              <div className="w-20 h-20 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 group-hover:scale-110 transition-transform">
                <svg
                  className="w-10 h-10"
                  fill="none"
                  stroke="currentColor"
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {selectedFiles.map((item, idx) => (
                  <div
                    key={item.id}
                    className={`relative rounded-2xl overflow-hidden border-2 transition-all duration-300 bg-white group shadow-sm flex flex-col h-full ${item.status === "processing" ? "border-indigo-400 ring-4 ring-indigo-50 shadow-indigo-100" : item.status === "error" ? "border-red-200" : "border-slate-100"}`}
                  >
              ...
              <div className="pt-4">
                <button
                  ...
                  className={`w-full font-black py-5 rounded-2xl shadow-2xl transition-all flex items-center justify-center gap-4 text-xl ${loading ? "bg-slate-100 text-slate-400 border border-slate-200 shadow-none" : "bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-200"}`}
                >
                  {loading ? (
